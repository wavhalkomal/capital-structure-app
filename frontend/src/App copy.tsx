...
export default function App() {
  ...
  const missing = useMemo(() => {
    ...
    if (!marketCap.trim()) m.push('market cap ($mm)')
    return m
  }, [balanceSheet, debtNote, leaseNote, metadata, marketCap])
  ...
  async function createJob() {
    ...
    try {
      ...
      fd.append('market_cap_mm', marketCap)
      ...
      if (!resp.ok) {
        ...
        throw new Error(err.detail || `Failed creating job (${resp.status})`)
      }
    ...
    <div style={{ maxWidth: 1100, margin: '0 auto', padding: 20 }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr', gap: 14, marginTop: 16 }}>
        <section style={{ background: 'white', border: '1px solid #e6e6ef', borderRadius: 16, padding: 16 }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12, marginTop: 12 }}>
            <div>
              <label style={{ fontSize: 13, color: '#444' }}>Market Cap ($mm)</label>
              <input
                value={marketCap}
